...
(define-test string-functions
  ...
  (is equal (let ((s (make-string-output-stream)))
	      (indent-string 3 s)
	      (get-output-stream-string s))
      "   ")
  (true (empty-string/newline-p (let ((s (make-string-output-stream)))
				  (write-string "    " s)
				  (write-char #\newline s)
				  (get-output-stream-string s))))
  (false (empty-string/newline-p (let ((s (make-string-output-stream)))
